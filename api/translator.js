export default async function handler(req, res) {
  ...
  if (!to) {
    return res.status(400).json({ status: 400, creator: 'RyodevAPI', error: 'Parameter "to" is required (e.g. id, en, ja, ko, ar)' });
  }
  ...
  const fromLang = from === 'auto' ? 'auto-detect the source language' : (langNames[from] || from);
